...
export default class DefinitionAdapter {
  ...
  async getDefinition(
    ...
    const definitions = DefinitionAdapter.normalizeLocations(await connection.gotoDefinition(documentPositionParams));
    if (definitions == null || definitions.length === 0) { return null; }

    return DefinitionAdapter.definitionsToQueryResult(ranges, definitions, this._languageName);
  }

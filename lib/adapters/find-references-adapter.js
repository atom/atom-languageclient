...
export default class FindReferencesAdapter {
  ...
  async getReferences(connection: LanguageClientConnection, editor: atom$TextEditor, point: atom$Point, projectRoot: ?string): Promise<?nuclide$FindReferencesReturn> {
    const locations = await connection.findReferences(FindReferencesAdapter.createDocumentPositionParams(editor, point));
  ...
  // Public: Create a {DocumentPositionParams} from a given {TextEditor} for a specific {Point}.
  //
  ...
  static createDocumentPositionParams(editor: atom$TextEditor, point: atom$Point): ls.DocumentPositionParams {
    return {

...
export default class FormatCodeAdapter {
  ...
  registerCommands(capabilities: ServerCapabilities, textEditorScopes: string) {
    if (capabilities.documentRangeFormattingProvider === true) {
      this._disposable.add(atom.commands.add(textEditorScopes, {'language:format-selection': this.formatSelection.bind(this)}));
    };
    if (capabilities.documentFormattingProvider === true) {
      this._disposable.add(atom.commands.add(textEditorScopes, {'language:format-file': this.formatDocument.bind(this)}));
    };
  }

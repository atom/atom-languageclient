...
export default class AutoLanguageClient {
  ...
  captureServerErrors(process: child_process$ChildProcess): void {
    process.on('error', err => this.logger.warn('process error', err));
    process.on('exit', (code, signal) => this.logger.debug(`exited code ${code}`));
    process.stderr.on('data', (chunk: Buffer) => this.logger.warn('stderr', chunk.toString()));
  }

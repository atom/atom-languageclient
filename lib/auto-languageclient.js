...
export default class AutoLanguageClient {
  ...
  areCurrentSuggestionsComplete: boolean = false;
  ...
  async getSuggestions(request: atom$AutocompleteRequest): Promise<Array<atom$AutocompleteSuggestion>> {
    ...
    if (
      prefix.length > 0 &&
      triggerChars.indexOf(prefix) === -1 &&
      this.currentSuggestions &&
      this.currentSuggestions.length > 0 &&
      this.areCurrentSuggestionsComplete
    ) {
      // fuzzy filter on this.currentSuggestions
      return new Promise(resolve => {
        const filtered = filter(this.currentSuggestions, prefix, {key: 'text'}).map(s => ({
          ...s,
    ...
    return this.autoComplete.getSuggestions(server.connection, request).then(response => {
      this.currentSuggestions = response.completionItems;
      this.areCurrentSuggestionsComplete = response.isComplete;

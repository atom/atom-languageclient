...
export default class AutoLanguageClient {
  ...
  shouldSyncForEditor(editor: atom$TextEditor, projectPath: string): atom$TextEditor => boolean {
    return editor => this._isFileInProject(editor, projectPath) && this._shouldStartForEditor(editor);
  }

  _isFileInProject(editor: atom$TextEditor, projectPath: string): boolean {
    return (editor.getURI() || '').startsWith(projectPath);
  }

  _shouldStartForEditor(editor: atom$TextEditor): boolean {

...
import LinterPullV1Adapter from './adapters/linter-pull-v1-adapter';
...
export default class AutoLanguageClient {
  ...
  async startExclusiveAdapters(server: ActiveServer): Promise<void> {
    ...
    if (this._indie != null) {
      server.linterPushV2.attach(this._indie);
    }
  }
  ...
  // Linter pull v1 API via LS publishDiagnostics
  ...
  async getLinting(editor: atom$TextEditor): Promise<?Array<linter$Message>> {
    ...
    if (server.linterPullV1 == null) {
      server.linterPullV1 = new LinterPullV1Adapter(server.connection);
    }

    return server.linterPullV1.provideDiagnostics();
  }

  // Linter push v2 API via LS publishDiagnostics

  consumeLinterV2(registerIndie: () => linter$V2IndieDelegate): void {
    this._indie = registerIndie({name: this.name});
    if (this._indie == null) return;

...
export default class AutoLanguageClient {
  ...
  getName() { throw "Must set name field when extending Autoadapter" };
  getGrammarScopes() { throw "Must set grammarScopes field when extending Autoadapter" };
  ...
  async startServer(): Promise<void> {
    ...
    this.adapterCapabilities(initializeResponse.capabilities);
  ...
  adapterCapabilities(capabilities: ls.ServerCapabilities): void {
    ...
    if (capabilities.documentSymbolProvider) {
      this.outlineView = new NuclideOutlineViewAdapter(this._lc, this.getName());
    }
    if (capabilities.referencesProvider) {
      this.findReferences = new NuclideFindReferencesAdapter(this._lc);
    }

    new NotificationsAdapter(this._lc, this.getName());
    ...
    if (capabilities.documentRangeFormattingProvider || capabilities.documentFormattingProvider) {
      this._disposable.add(new FormatCodeAdapter(this._lc, capabilities.documentRangeFormattingProvider === true, capabilities.documentFormattingProvider === true));
    }
  }

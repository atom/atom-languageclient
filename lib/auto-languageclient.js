...
import AutocompleteAdapter, {type AutocompletionResponse} from './adapters/autocomplete-adapter';
...
type CompletionResponseCache = {
  prefixPosition: atom$PointObject,
...
export default class AutoLanguageClient {
  ...
  async getSuggestions(request: atom$AutocompleteRequest): Promise<Array<atom$AutocompleteSuggestion>> {
    ...
    let prefixPosition: atom$PointObject;
    ...
    if (triggerChars.includes(request.prefix)) {
      prefixPosition = request.bufferPosition;
    } else {
      prefixPosition = {
    ...
    const triggerChar: string = request.editor
      .getBuffer()
      .getTextInRange([[prefixPosition.row, prefixPosition.column - 1], prefixPosition]);
    ...
    if (
      completionCache.prefixPosition.row === prefixPosition.row &&
      completionCache.prefixPosition.column === prefixPosition.column &&
      ...
      let k: number = request.prefix.length;
      ...
      while (!resultsForPrefix && k >= 0) {
        ...
        if (k === 0) {
          _resultsForPrefix = completionCache.prefixes.get(triggerChar);
        } else {
          _resultsForPrefix = completionCache.prefixes.get(request.prefix.slice(0, k));
        }
        ...
        if (_resultsForPrefix) {
          ...
          if (_resultsForPrefix.isComplete) {
            ...
            isFilterRequired = k !== request.prefix.length;
          }
      ...
      if (resultsForPrefix && resultsForPrefix.completionItems) {
        return new Promise(resolve => {
          ...
          const filtered = isFilterRequired ? filter(listToFilter, request.prefix, {key: 'text'}).map(s => ({
            ...s,
    ...
    return this.autoComplete.getSuggestions(server.connection, request).then(response => {
      ...
      completionCache.prefixes.set(request.prefix, response);

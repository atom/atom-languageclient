...
export default class AutoLanguageClient {
    ...
    if (DocumentSyncAdapter.canAdapt(server.capabilities)) {
      server.disposable.add(
        new DocumentSyncAdapter(server.connection, server.capabilities.textDocumentSync,
          this.newEditorSelector(editor, server.projectPath)));
    }
  ...
  newEditorSelector(editor: atom$TextEditor, basepath: string): atom$TextEditor => boolean {
    return editor => this._isFileInProject(editor, basepath) && this._isSupportedLanguage(editor);
  }

  _isFileInProject(editor: atom$TextEditor, basepath: string): boolean {
    return (editor.getURI() || '').startsWith(basepath);
  }

  _isSupportedLanguage(editor: atom$TextEditor): boolean {
